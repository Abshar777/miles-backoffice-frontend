      ...
      <div className="relative flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        {QUICK_REACTIONS.map(e => (
          <button key={e} type="button" title={`React ${e}`} onClick={() => onReact(e)}
            className="text-[13px] leading-none px-1 py-0.5 rounded hover:bg-slate-200/70">{e}</button>
        ))}
        <button type="button" title="More emojis" onClick={() => setPickerOpen(o => !o)}
          className="w-5 h-5 flex items-center justify-center rounded-full border border-slate-300 text-slate-500 hover:bg-slate-100 text-xs leading-none">＋</button>
        {pickerOpen && (
          <>
