              ...
              {activeSection === 'channels' && selectedChannel ? (
                <>
                    ...
                    {channelMessages.length === 0 ? (
                      ...
                      groupedChannelMessages.map(msg => (
                        <div key={msg.msg_id}>
                          ...
                          <div className="group flex gap-3 px-4 hover:bg-slate-50/80 py-0.5 transition-colors">
                            {/* Avatar or spacer */}
                            ...
                            <div className="flex-1 min-w-0 pb-0.5">
                              {!msg.isGrouped && (
                                <div className="flex items-baseline gap-2 mb-0.5">
                                  <span className="text-sm font-bold text-slate-900">{msg.sender_id === user?.user_id ? 'You' : msg.sender_name}</span>
                                  ...
                                  <button
                                    ...
                                    className="opacity-0 group-hover:opacity-100 transition-opacity ml-1 flex items-center gap-1 text-xs text-slate-400 hover:text-blue-600 bg-white hover:bg-blue-50 rounded px-2 py-0.5 border border-slate-200 hover:border-blue-300 shadow-sm"
                                  >
                              ...
                              {msg.isGrouped && (
                                <button
                                  onClick={() => { setThreadMsg(msg); if (selectedChannel) fetchThreadReplies(selectedChannel.channel_id, msg.msg_id); }}
                                  className="opacity-0 group-hover:opacity-100 transition-opacity float-right flex items-center gap-1 text-xs text-slate-400 hover:text-blue-600 bg-white hover:bg-blue-50 rounded px-2 py-0.5 border border-slate-200 hover:border-blue-300 shadow-sm -mt-0.5"
                                >
                                  <MessageCircle className="w-3 h-3" /> Reply
                                </button>
                              )}
                              {msg.content && <p className="text-sm text-slate-800 whitespace-pre-wrap leading-relaxed">{msg.content}</p>}
                              {renderAttachments(msg.attachments, msg.sender_id === user?.user_id)}
                              {msg.reply_count > 0 && (
                                <button
                                  onClick={() => { setThreadMsg(msg); if (selectedChannel) fetchThreadReplies(selectedChannel.channel_id, msg.msg_id); }}
                                  className="flex items-center gap-1.5 mt-1.5 text-xs text-blue-600 hover:underline font-medium">
                  ...
                  <div className="px-4 py-3 border-t shrink-0">
                    ...
                    <div className="flex items-end gap-2 border border-slate-200 rounded-xl px-3 py-2 focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-100 transition-all bg-white">
            ...
            {threadMsg && selectedChannel && (
              <Card className="w-80 shrink-0 flex flex-col min-h-0">
                ...
                <div className="px-3 py-3 border-t shrink-0">
                  ...
                  <div className="flex items-end gap-2 border border-slate-200 rounded-xl px-2.5 py-1.5 focus-within:border-blue-400 transition-all bg-white">
