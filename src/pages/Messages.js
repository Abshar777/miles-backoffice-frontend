        ...
        {others.map((f, i) => (
          <a key={i} href={f.url} target="_blank" rel="noopener noreferrer"
            className={`flex items-center gap-2 p-2 rounded-lg ${
              isSelf ? 'bg-blue-500/20 hover:bg-blue-500/30' : 'bg-slate-200 hover:bg-slate-300'
            }`}
          >
            {getFileIcon(f.filename, f.content_type)}
            <div className="min-w-0">
              <p className="text-xs font-medium truncate">{f.filename}</p>
              <p className={`text-xs ${isSelf ? 'text-blue-200' : 'text-slate-400'}`}>{formatFileSize(f.size)}</p>
            </div>
          </a>
        ))}
      </div>
        ...
        {files.map((f, i) => (
          <div key={i} className="relative group">
            {isImage(f.name, f.type) ? (
              <div className="w-14 h-14 rounded overflow-hidden border border-slate-200">
    ...
    <div className="h-[calc(100vh-120px)] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div>
          ...
          <p className="text-slate-500 mt-1">Direct messages &amp; group channels</p>
        </div>
        {isAdmin && (
      ...
      {isAdmin && viewMode === 'all' ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-1 min-h-0">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-800 truncate">{conv.user1_name} &amp; {conv.user2_name}</p>
          ...
          <Card className="md:col-span-2 flex flex-col">
              ...
              {selectedAllConversation ? (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Viewing conversation between users</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    {allMessages.map((msg, idx) => (
                      <div key={idx} className={`flex ${msg.sender_id === selectedAllConversation.user1_id ? 'justify-start' : 'justify-end'}`}>
                        <div className={`max-w-[70%] ${msg.sender_id === selectedAllConversation.user1_id ? 'bg-slate-100' : 'bg-purple-100'} rounded-lg px-4 py-2`}>
            ...
            {selectedAllConversation && <div className="p-3 border-t bg-slate-50"><p className="text-xs text-slate-500 text-center">Admin view – Read only</p></div>}
          </Card>
        </div>
      ) : (
            ...
            <ScrollArea className="flex-1">
              ...
              <div className="p-2">
                <div className="flex items-center justify-between px-2 py-1 mb-1">
                  ...
                  <button onClick={() => setNewChatDialog(true)} className="flex items-center gap-0.5 text-xs text-blue-600 hover:text-blue-800 font-medium" title="New message">
                    <Plus className="w-3 h-3" /> New
                  </button>
                </div>
                {filteredConversations.length === 0 && (
                  <button onClick={() => setNewChatDialog(true)} className="w-full text-left px-2 py-2 text-xs text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-md transition-colors">+ Start a conversation</button>
                )}
                {filteredConversations.map(conv => (
                  <div key={conv.user_id}
                    ...
                    className={`flex items-center gap-2 px-2 py-2 rounded-md cursor-pointer transition-colors ${
                      activeSection === 'dm' && selectedConversation?.user_id === conv.user_id
                        ? 'bg-blue-50 text-blue-700' : 'text-slate-700 hover:bg-slate-100'
                    }`}
                  ...
                  >
                    ...
                    {conv.unread_count > 0 && (
                      <Badge className="bg-blue-600 text-white text-xs h-5 px-1.5 shrink-0">{conv.unread_count}</Badge>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="p-2 border-t">
                <div className="flex items-center justify-between px-2 py-1 mb-1">
                  ...
                  <button onClick={() => setNewChannelDialog(true)} className="flex items-center gap-0.5 text-xs text-blue-600 hover:text-blue-800 font-medium" title="New channel">
                    <Plus className="w-3 h-3" /> New
                  </button>
                </div>
                {filteredChannels.length === 0 && (
                  <button onClick={() => setNewChannelDialog(true)} className="w-full text-left px-2 py-2 text-xs text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-md transition-colors">+ Create a channel</button>
                )}
                {filteredChannels.map(ch => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {ch.last_message && <p className="text-xs text-slate-400 truncate">{ch.last_message}</p>}
                    </div>
                    {ch.unread_count > 0 && (
                      <Badge className="bg-blue-600 text-white text-xs h-5 px-1.5 shrink-0">{ch.unread_count}</Badge>
                    )}
                  </div>
                ))}
              </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {selectedChannel.description && (
                          <p className="text-xs text-slate-500 truncate">{selectedChannel.description}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-baseline gap-2 mb-0.5">
                                  ...
                                  <button
                                    ...
                                    className="ml-1 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-xs text-slate-400 hover:text-blue-600 bg-slate-100 hover:bg-blue-50 rounded px-2 py-0.5 border border-transparent hover:border-blue-200"
                                  >
                                ...
                                {msg.reply_count > 0 && (
                                  <button
                                    onClick={() => openThread(msg)}
                                    className="flex items-center gap-1 mt-1 text-xs text-blue-600 hover:text-blue-800 hover:underline font-medium"
                                  >
                        ...
                        {messages.length === 0 ? (
                          ...
                          <div key={msg.message_id} className={`flex ${msg.sender_id === user?.user_id ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[70%] rounded-2xl px-4 py-2 ${
                                ...
                                isImage(msg.attachment.filename, msg.attachment.content_type) ? (
                                  ...
                                  <a href="#" onClick={async e => {
                                    ...
                                    className={`flex items-center gap-2 mt-1 p-2 rounded-lg cursor-pointer ${
                                      msg.sender_id === user?.user_id ? 'bg-blue-500/30 hover:bg-blue-500/50' : 'bg-slate-200 hover:bg-slate-300'
                                    }`}
                                  >
                              ...
                              <div className={`flex items-center justify-end gap-1 mt-1 ${msg.sender_id === user?.user_id ? 'text-blue-200' : 'text-slate-400'}`}>
                ...
                <CardHeader className="pb-2 border-b shrink-0 py-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">#{selectedChannel.name}</p>
                    </div>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline gap-2">
                            <span className="text-xs font-semibold text-slate-800">{threadMsg.sender_name}</span>
                            <span className="text-xs text-slate-400">{formatTime(threadMsg.created_at)}</span>
                          </div>
                          {threadMsg.content && <p className="text-sm text-slate-700 mt-0.5 whitespace-pre-wrap">{threadMsg.content}</p>}
                    ...
                    {threadReplies.map(reply => (
                      <div key={reply.msg_id} className="flex items-start gap-2 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {reply.content && <p className="text-sm text-slate-700 mt-0.5 whitespace-pre-wrap">{reply.content}</p>}
