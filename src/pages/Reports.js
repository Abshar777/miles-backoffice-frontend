            ...
            <>
              ...
              <Card className="bg-white border-slate-200 shadow-sm">
                <CardHeader className="pb-3 flex flex-row items-center justify-between">
                  <CardTitle className="text-lg font-semibold text-slate-900">Exchanger Transactions</CardTitle>
                  <Button size="sm" variant="ghost" className="text-slate-400 hover:text-slate-700" onClick={exportVendorTransactions}>
                    <Download className="w-4 h-4 mr-2" /> Export CSV
                  </Button>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        className="pl-7 h-8 text-xs border-slate-200 w-48"
                    ...
                    <Select value={vtFilter.vendor_id} onValueChange={v => { const f = { ...vtFilter, vendor_id: v }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                      <SelectTrigger className="w-40 h-8 text-xs border-slate-200"><SelectValue placeholder="Exchanger" /></SelectTrigger>
                      <SelectContent>
                    ...
                    <Select value={vtFilter.category} onValueChange={v => { const f = { ...vtFilter, category: v, destination_id: 'all' }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                      <SelectTrigger className="w-36 h-8 text-xs border-slate-200"><SelectValue placeholder="Category" /></SelectTrigger>
                      <SelectContent>
                    ...
                    {vtFilter.category === 'psp' && (
                      <Select value={vtFilter.destination_id} onValueChange={v => { const f = { ...vtFilter, destination_id: v }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                        <SelectTrigger className="w-40 h-8 text-xs border-slate-200"><SelectValue placeholder="All PSPs" /></SelectTrigger>
                        <SelectContent>
                    ...
                    {(vtFilter.category === 'treasury' || vtFilter.category === 'usdt') && (
                      <Select value={vtFilter.destination_id} onValueChange={v => { const f = { ...vtFilter, destination_id: v }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                        <SelectTrigger className="w-44 h-8 text-xs border-slate-200"><SelectValue placeholder="All Accounts" /></SelectTrigger>
                        <SelectContent>
                    ...
                    <Select value={vtFilter.transaction_type} onValueChange={v => { const f = { ...vtFilter, transaction_type: v }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                      <SelectTrigger className="w-32 h-8 text-xs border-slate-200"><SelectValue placeholder="Type" /></SelectTrigger>
                      <SelectContent>
                    ...
                    <Select value={vtFilter.status} onValueChange={v => { const f = { ...vtFilter, status: v }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                      <SelectTrigger className="w-32 h-8 text-xs border-slate-200"><SelectValue placeholder="Status" /></SelectTrigger>
                      <SelectContent>
                    ...
                    <Button size="sm" variant="outline" className="h-8 text-xs border-slate-200" onClick={() => { const f = { vendor_id: 'all', category: 'all', transaction_type: 'all', status: 'all', search: '', destination_id: 'all' }; setVtFilter(f); setVtPage(1); fetchVendorTransactions(f, 1); }}>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-slate-200">
                          ...
                          <TableHead className="text-slate-500 text-xs">Destination</TableHead>
                          <TableHead className="text-slate-500 text-xs text-right">Amount</TableHead>
                          <TableHead className="text-slate-500 text-xs text-right">Commission</TableHead>
                          <TableHead className="text-slate-500 text-xs">Status</TableHead>
                          <TableHead className="text-slate-500 text-xs">Tags</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {vtLoading ? (
                          ...
                          <TableRow><TableCell colSpan={10} className="text-center text-slate-500 py-8">No transactions found</TableCell></TableRow>
                        ) : (vtData?.items || []).map(tx => (
                          <TableRow key={tx.transaction_id} className="border-slate-200 hover:bg-slate-50">
                            <TableCell className="text-xs text-slate-500">{tx.transaction_date || tx.created_at?.slice(0,10)}</TableCell>
                            <TableCell className="text-xs text-slate-900 max-w-[120px] truncate">{tx.client_name}</TableCell>
                            <TableCell className="text-xs text-slate-700">{tx.vendor_name}</TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                              <Badge variant="outline" className="text-xs border-slate-200 text-slate-500 capitalize">{tx.destination_type}</Badge>
                            </TableCell>
                            <TableCell className="text-xs text-slate-600 max-w-[120px] truncate">{tx.psp_name || tx.destination_account_name || '—'}</TableCell>
                            <TableCell className={`text-xs font-mono text-right ${tx.transaction_type === 'deposit' ? 'text-emerald-500' : 'text-red-500'}`}>
                              ...
                              {tx.base_currency && tx.base_currency !== 'USD' && <div className="text-[10px] text-slate-400">{tx.base_amount?.toLocaleString()} {tx.base_currency}</div>}
                            </TableCell>
                            ...
                            <TableCell className="text-xs">
                              <div className="flex flex-wrap gap-1">
                                {(tx.transaction_tags || []).map((tag, i) => <Badge key={i} variant="outline" className={`text-[10px] px-1 ${tag === 'Edited' ? 'border-orange-300 text-orange-600 bg-orange-50' : 'border-slate-200 text-slate-400'}`}>{tag}</Badge>)}
                              </div>
                            </TableCell>
                          </TableRow>
                        ))}
                  ...
                  {vtData && vtData.total_pages > 1 && (
                    <div className="flex items-center justify-between pt-2">
                      <span className="text-xs text-slate-500">Page {vtData.page} of {vtData.total_pages} · {vtData.total} records</span>
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" className="h-7 px-2 text-xs border-slate-200" disabled={vtPage <= 1} onClick={() => { const p = vtPage - 1; setVtPage(p); fetchVendorTransactions(vtFilter, p); }}>Prev</Button>
                        <Button size="sm" variant="outline" className="h-7 px-2 text-xs border-slate-200" disabled={vtPage >= vtData.total_pages} onClick={() => { const p = vtPage + 1; setVtPage(p); fetchVendorTransactions(vtFilter, p); }}>Next</Button>
                      </div>
                    </div>
                  )}
                </CardContent>
              ...
              <Card className="bg-white border-slate-200 shadow-sm">
                ...
                <CardContent>
                  <Table>
                    <TableHeader>
                      <TableRow className="border-slate-200">
                        <TableHead className="text-slate-500 text-xs w-6"></TableHead>
                    ...
                    <TableBody>
                      {(treasuryReport.accounts || []).map((acc, i) => {
                        ...
                        return (
                          <>
                            {/* Account row */}
                            <TableRow key={`acc-${i}`} className="border-slate-200 hover:bg-slate-50 cursor-pointer"
                              ...
                              <TableCell>
                                <div className="flex items-center gap-2">
                                  ...
                                  {accTxs.length > 0 && (
                                    <span className="text-xs text-indigo-500 font-medium">{accTxs.length} txn{accTxs.length > 1 ? 's' : ''}</span>
                                  )}
                                </div>
                              </TableCell>
                            </TableRow>
                            ...
                            {isExpanded && (
                              <TableRow key={`acc-expand-${i}`} className="bg-slate-50 border-slate-200">
                                <TableCell colSpan={7} className="p-0">
                                  <div className="px-8 py-3 border-l-4 border-indigo-400">
                                    {/* Summary bar */}
                                    <div className="flex items-center gap-6 mb-3 text-xs">
                                      <span className="font-semibold text-slate-600">{accTxs.length} transaction{accTxs.length !== 1 ? 's' : ''} {dateFrom || dateTo ? 'in selected period' : ''}</span>
                                      {totalIn  > 0 && <span className="text-emerald-600 font-mono">+${totalIn.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})} in</span>}
                                      {totalOut > 0 && <span className="text-red-500 font-mono">-${totalOut.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})} out</span>}
                                    </div>
                                    {accTxs.length === 0 ? (
                                      <p className="text-xs text-slate-400 py-2">No transactions for this account{dateFrom || dateTo ? ' in the selected period' : ''}.</p>
                                    ) : (
                                      <table className="w-full text-xs">
                                        ...
                                        <tbody>
                                          {accTxs.map((tx, j) => (
                                            <tr key={j} className="border-b border-slate-100 hover:bg-white">
                                              <td className="py-1.5 font-mono text-slate-600 truncate max-w-[120px]">{tx.reference || tx.transaction_id?.slice(-8)}</td>
                                              <td className="py-1.5 text-slate-700">{tx.client_name || '-'}</td>
                                              <td className="py-1.5">
                                              ...
                                              <td className={`py-1.5 font-mono text-right ${tx.transaction_type === 'deposit' ? 'text-emerald-600' : 'text-red-500'}`}>
                                                {tx.transaction_type === 'deposit' ? '+' : '-'}${(tx.amount || 0).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}
                                              </td>
                                              <td className="py-1.5 font-mono text-right text-slate-500">
                                              ...
                                              <td className="py-1.5 text-slate-500">{tx.transaction_date || tx.created_at?.slice(0,10) || '-'}</td>
                                            </tr>
                                          ))}
                                        </tbody>
                                      </table>
