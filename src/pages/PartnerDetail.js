                ...
                {txLoading ? (
                  ...
                  txItems.map((tx) => (
                    <TableRow key={tx.transaction_id}>
                      ...
                      <TableCell className="text-xs text-slate-500" data-testid={`pd-completed-date-${tx.transaction_id}`}>
                        {tx.completed_at
                    ...
                    <div key={group.destination_type} className="space-y-2" data-testid={`ptreasury-group-${group.destination_type}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {group.entries.map((entry) => (
                          <Card
                            key={entry.key}
                            className="bg-white border-slate-200 hover:border-blue-300 hover:shadow-sm transition-all cursor-pointer relative overflow-hidden"
                          ...
                          >
                            ...
                            {entry.is_hidden && !revealedEntries.has(entry.key) && (
                              <button
                                ...
                                className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-1.5 bg-white/50 backdrop-blur-[1px] cursor-pointer"
                                data-testid={`ptreasury-reveal-${entry.key}`}
                              >
                                <EyeOff className="w-5 h-5 text-slate-400" />
                                <span className="text-xs text-slate-500 font-medium">Hidden &mdash; click to reveal</span>
                              </button>
                            )}
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  {canEditCharges && (
                                    <button
                                      ...
                                      className="h-9 w-9 rounded-lg border text-slate-400 hover:text-slate-700 flex items-center justify-center transition-colors"
