  ...
  <Card className="bg-white border-slate-200 shadow-sm">
    <CardContent className="p-4">
      <div className="flex items-start justify-between">
        ...
        <div className={`p-2 rounded-lg ${color === 'green' ? 'bg-green-100' : color === 'red' ? 'bg-red-100' : color === 'yellow' ? 'bg-yellow-100' : 'bg-blue-100'}`}>
          <Icon className={`w-5 h-5 ${color === 'green' ? 'text-green-600' : color === 'red' ? 'text-red-600' : color === 'yellow' ? 'text-yellow-600' : 'text-blue-600'}`} />
        </div>
      </div>
    </CardContent>
  </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      {partners.length === 0 ? (
        <Card className="bg-white border-slate-200">
          <CardContent className="p-10 text-center text-slate-400">
            ...
            <p className="font-medium text-slate-600">No partners visible</p>
      ...
      <Dialog open={!!viewPartner} onOpenChange={(open) => { if (!open) closePartner(); }}>
        <DialogContent className="bg-white border-slate-200 text-slate-800 max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 rounded-lg p-3">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{viewPartner.deposit_count} txns</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3">
                  <p className="text-xs text-slate-400 uppercase tracking-wider">Withdrawals</p>
                  <p className="text-base font-mono font-semibold text-red-600">{fmtUsd(viewPartner.total_withdrawals_usd)}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{viewPartner.withdrawal_count} txns</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3">
