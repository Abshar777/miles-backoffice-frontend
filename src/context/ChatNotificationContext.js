    ...
    <ChatNotificationContext.Provider value={{
      ...
      {incomingBuzz && (
        <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-6 px-4 pointer-events-none">
          <div className="pointer-events-auto w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 pt-4">
              <div className="w-11 h-11 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center animate-pulse shrink-0">
                <span className="text-xl">📞</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-slate-800 leading-tight truncate">{incomingBuzz.from_name} is buzzing…</p>
                <p className="text-xs text-slate-500 truncate">{incomingBuzz.scope === 'channel' ? `# ${incomingBuzz.channel_name}` : 'Direct message'}</p>
              </div>
            </div>
            {incomingBuzz.reason && <p className="px-4 pt-2 text-sm text-slate-600 italic break-words">"{incomingBuzz.reason}"</p>}
            <div className="flex gap-2 p-4">
              <button onClick={declineBuzz} className="flex-1 h-9 rounded-lg bg-slate-100 text-slate-600 text-sm font-medium hover:bg-slate-200 transition-colors">Decline</button>
              <button onClick={answerBuzz} className="flex-1 h-9 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors">Answer</button>
            </div>
          </div>
        </div>
      )}
